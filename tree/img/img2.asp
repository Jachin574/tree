...
if upload.form("act")="uploadfile" then
 ...
 if filelx="jpg" then
	if file.filesize>(2000*1024) then
		response.write "<script language=javascript>alert('图片文件大小不能超过2m！');history.go(-1);</script>"
		response.end
	end if
 end if
 ...
 filename=filepath&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&ranNum&"."&fileExt'保存到数据库的图片名字
%>
<%
